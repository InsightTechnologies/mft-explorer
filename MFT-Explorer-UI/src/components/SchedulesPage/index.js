...
class CustomPaginationActionsTable extends React.Component {
  ...
  searchChangeHandler = event => {
    ...
    if (event.target.value === "") {
      filteredRows = this.state.schedules;
    } else {
      const schedulesFromState = this.state.schedules.slice();
      const searchWord = event.target.value;

      filteredRows = schedulesFromState.filter(obj => {
        let match=false;
  ...
  fetchSchedules = (qm) => {
    ...
    const url = `${Endpoints.Schedules}/${qm.host_ip}:${qm.qmChannel}:${qm.qmPort}`
    RestClient.Get(url, {},
      data => {
        if (data.Success) {
          let result=[];
          ...
          this.refs.utils.Error("Failed to fetch schedule details")
        }
        this.refs.utils.Loading(false)
      },
      error => {
  ...
  componentDidMount() {
    this.fetchCordManagerAndSchedules()
  }
 ...
 return result?result:"--";
    ...
    return (
      <React.Fragment>
        <Utils ref='utils' />
        <div>
          <Typography variant='h5'>Scheduled Transfers</Typography>
        </div>
